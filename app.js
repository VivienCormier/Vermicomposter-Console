...
app.get('/temperature/', function (req, res) {
  ...
  nosql.sort(function(data) {
    ...
    data.forEach(function (row) {
      if (row.date > before_date) {
        full_data.push(row);
        if (row.temp_level_1 && row.temp_level_1 > -10 && row.temp_level_1 < 50) {
          var new_row = JSON.parse(JSON.stringify(row));
        ...
        if (row.temp_level_2 && row.temp_level_2 > -10 && row.temp_level_2 < 50) {
          var new_row = JSON.parse(JSON.stringify(row));
        ...
        if (row.temp_level_3 && row.temp_level_3 > -10 && row.temp_level_3 < 50) {
          var new_row = JSON.parse(JSON.stringify(row));
